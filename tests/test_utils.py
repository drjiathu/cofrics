...
from cofrics.utils import (
    ...
    compute_forward_returns, quantize_factor
)
...
class TestUtils:
    ...
    factor_data['group'] = pd.Series(index=factor.index, data=[1, 1, 2, 2, 1, 1, 2, 2], dtype="category")
    ...
    biased_factor_data['group'] = pd.Series(index=biased_factor.index, data=[1, 1, 2, 2] * 4, dtype="category")

    @staticmethod
    def test_compute_forward_returns():
        dr = pd.date_range(start='2015-1-1', end='2015-1-3')
        prices = pd.DataFrame(index=dr, columns=['A', 'B'], data=[[1, 1], [1, 2], [2, 1]])
        factor = prices.stack()
        ...
        expected['1D'] = [0., 1., 1., -0.5, np.nan, np.nan]
        expected['2D'] = [1., 0., np.nan, np.nan, np.nan, np.nan]
    ...
    def test_compute_forward_returns_index_out_of_bound():
        ...
        prices = pd.DataFrame(
            index=dr,
            columns=['A', 'B'],
            data=[[np.nan, np.nan], [np.nan, np.nan], [np.nan, np.nan], [1, 1], [1, 2], [2, 1]]
        )
        dr = pd.date_range(start='2015-1-1', end='2015-1-3')
        factor = pd.DataFrame(index=dr, columns=['A', 'B'], data=[[1, 1], [1, 2], [2, 1]])
        factor = factor.stack()
        fp = compute_forward_returns(factor, prices, periods=[1, 2])
        ix = pd.MultiIndex.from_product([dr, ['A', 'B']], names=['date', 'asset'])
        expected = pd.DataFrame(index=ix, columns=['1D', '2D'])
    ...
    def test_compute_forward_returns_non_cum():
        ...
        prices = pd.DataFrame(index=dr, columns=['A', 'B'], data=[[1, 1], [1, 2], [2, 1]])
        factor = prices.stack()
        fp = compute_forward_returns(factor, prices, periods=[1, 2], cumulative_returns=False)
        ix = pd.MultiIndex.from_product([dr, ['A', 'B']], names=['date', 'asset'])
        expected = pd.DataFrame(index=ix, columns=['1D', '2D'])
        expected['1D'] = [0., 1., 1., -0.5, np.nan, np.nan]
        expected['2D'] = [1., -0.5, np.nan, np.nan, np.nan, np.nan]
    ...
    @pytest.mark.parametrize(
        ...
        [(factor_data, 4, None, False, False, [1, 2, 3, 4, 4, 3, 2, 1]),
         ...
         (biased_factor_data, 4, None, False, True, [2, 3, 2, 3, 1, 4, 1, 4, 2, 3, 2, 3, 1, 4, 1, 4]),
         (biased_factor_data, 2, None, False, True, [1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2]),
         (biased_factor_data, 2, None, True, True, [1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2]),
         (biased_factor_data, None, 4, False, True, [2, 3, 2, 3, 1, 4, 1, 4, 2, 3, 2, 3, 1, 4, 1, 4]),
         (biased_factor_data, None, 2, False, True, [1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2]),
         (biased_factor_data, None, 2, True, True, [1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2]),
         (factor_data, [0, .25, .5, .75, 1.], None, False, False, [1, 2, 3, 4, 4, 3, 2, 1]),
         (factor_data, [0, .5, .75, 1.], None, False, False, [1, 1, 2, 3, 3, 2, 1, 1]),
         (factor_data, [0, .25, .5, 1.], None, False, False, [1, 2, 3, 3, 3, 3, 2, 1]),
         (factor_data, [0, .5, 1.], None, False, False, [1, 1, 2, 2, 2, 2, 1, 1]),
         (factor_data, [.25, .5, .75], None, False, False, [np.nan, 1, 2, np.nan, np.nan, 2, 1, np.nan]),
         (factor_data, [0, .5, 1.], None, True, False, [1, 2, 1, 2, 2, 1, 2, 1]),
         (factor_data, [.5, 1.], None, True, False, [np.nan, 1, np.nan, 1, 1, np.nan, 1, np.nan]),
         (factor_data, [0, 1.], None, True, False, [1, 1, 1, 1, 1, 1, 1, 1]),
         ...
         (factor_data, None, [0, 1, 2, 3, 5], False, False, [1, 2, 3, 4, 4, 3, 2, 1]),
         (factor_data, None, [1, 2, 3], False, False, [np.nan, 1, 2, np.nan, np.nan, 2, 1, np.nan]),
         (factor_data, None, [0, 2, 5], False, False, [1, 1, 2, 2, 2, 2, 1, 1]),
         (factor_data, None, [0.5, 2.5, 4.5], False, False, [1, 1, 2, 2, 2, 2, 1, 1]),
         (factor_data, None, [0.5, 2.5], True, False, [1, 1, np.nan, np.nan, np.nan, np.nan, 1, 1]),
         (factor_data, None, 2, True, False, [1, 2, 1, 2, 2, 1, 2, 1])]
    )
    def test_quantize_factor(self, factor, quantiles, bins, by_group, zero_aware, expected_vals):
        ...
        expected = pd.Series(index=factor.index, data=expected_vals, name='factor_quantile').dropna()
    ...
    def test_get_clean_factor_and_forward_returns_0():
        ...
        factor_data = [[3, 4, 2, 1, np.nan, np.nan],
                       [3, np.nan, np.nan, 1, 4, 2],
                       [3, 4, 2, 1, np.nan, np.nan]]  # 3 days
        ...
        factor = pd.DataFrame(index=factor_index, columns=tickers, data=factor_data).stack()
        ...
        expected_data = [[0.1, 0.21, 0.331, 3.0, 1, 3],
                         ...
                         [2.0, 8.00, 26.000, 2.0, 1, 2],
                         [-0.1, -0.19, -0.271, 1.0, 2, 1]]
        expected = pd.DataFrame(index=expected_idx, columns=expected_cols, data=expected_data)
    ...
    def test_get_clean_factor_and_forward_returns_2():
        ...
        prices = pd.concat(
            [today_open, today_open_1h, today_open_3h]).sort_index()
    ...
    def test_get_clean_factor_and_forward_returns_3():
        ...
        factor_data = [[1, np.nan, np.nan, np.nan, np.nan, 6],
        ...
        price_index = pd.date_range(
            start='2017-1-12', end='2017-1-23', freq='B')
        price_index.name = 'date'
        prices = pd.DataFrame(index=price_index, columns=tickers, data=price_data)
    ...
    def test_get_clean_factor_and_forward_returns_4(self):
        ...
        factor_data = [[3, 4, 2, 1, np.nan, np.nan],
                       [3, np.nan, np.nan, 1, 4, 2],
                       [3, 4, 2, 1, np.nan, np.nan]] * 6  # 18 days
